...
```sh
# 构建镜像
docker build -t kkbt/obcsapi:v4.2.1 . 
# 运行 Docker
docker run -d -p 8900:8900 --name myObcsapi4.2.1 -v /home/kkbt/app/obcsapi-go/:/app/data/ kkbt/obcsapi:v4.2.1
# 或者通过 cp 方式修改好的 config.yaml
docker cp config.yaml myObcsapi4.2.1:/app/data/config.yaml
docker restart myObcsapi4.2.1
```
