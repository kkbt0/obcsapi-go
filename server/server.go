...
// @externalDocs.url https://www.ftls.xyz/docs/obcsapi/
...
func main() {
	...
	r.Any("/api/wechat", wechat.WeChatMpHandlers) // wechat 机器人 用于公众测试号
	...
	apiGroup := r.Group("/api", TokenAuthMiddleware("./token/token2.json")) // default token2
	{
		...
		apiGroup.POST("workwechatmsg", WorkWeChatMpInfoHandler) // 企业微信通知
	}

	obGroup2 := r.Group("/ob", TokenAuthMiddleware("./token/token2.json")) // default token2
	{
		obGroup2.POST("fv", apps.FvHandler)         // Obsidian Token2 POST 安卓 FV 悬浮球 快捷存储 文字，图片
		obGroup2.POST("sr/webhook", apps.SRWebHook) // Obsidian Token2 POST 简悦 Webhook 使用
	...
	r.POST("/ob/moonreader", StandardTokenAuthMiddleware("./token/token2.json"), apps.MoodReaderHandler) // Obsidian POST 静读天下 api 此 API 使用 Authorization 头验证
